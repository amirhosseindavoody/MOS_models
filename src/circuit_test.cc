...
TEST_F(CircuitTest, AddNodes) {
  int n1 = CircuitAddNode(c, "1");
  int n2 = CircuitAddNode(c, "2");
  int n3 = CircuitAddNode(c, "out");
...
// Ground node always gets index 0
TEST_F(CircuitTest, GroundNodeHandling) {
  int g1 = CircuitAddNode(c, "0");
  int g2 = CircuitAddNode(c, "gnd");
  int g3 = CircuitAddNode(c, "GND");
  int g4 = CircuitAddNode(c, "ground");
...
TEST_F(CircuitTest, AddDevices) {
  ...
  Device* added = CircuitAddDevice(c, r);
...
TEST_F(CircuitTest, Finalization) {
  ...
  Device* r = CreateResistor("R1", n1, n2, 1000.0);
  CircuitAddDevice(c, r);

  int result = CircuitFinalize(c);
  EXPECT_EQ(result, 0);
...
TEST_F(CircuitTest, SimpleVoltageDivider) {
  ...
  int n1 = CircuitAddNode(c, "1");         // between R1 and R2
  int n_vpos = CircuitAddNode(c, "vpos");  // positive terminal of V1
  ...
  CircuitAddDevice(c, v1);
  CircuitAddDevice(c, r1);
  CircuitAddDevice(c, r2);
  ...
  double x[4] = {0};
  int iters = CircuitDcAnalysis(c, x, 100, 1e-9, 1e-6);
...
R2 out 0 1k
  ...
  int out_node_idx = CircuitGetNode(c, "out");
...
R1 out 0 1k
  ...
  int out_node = CircuitGetNode(c, "out");
  int node_1 = CircuitGetNode(c, "1");
